...
void minigame_detail_screen_create() {
  det_sc_time_string =
      mem_zone_alloc(&memory_pool, sizeof(char) * 9); // format Record: mm:ss\0
...
void minigame_detail_screen_display(display_context_t disp) {
  ...
  { // record
    ...
    graphics_draw_sprite_trans_stride(disp, RES_X / 2 - 14, SCREEN_TOP + 15,
                                      ui_sprites, SPRITE_record);
    graphics_draw_text(disp, RES_X / 2 + 4, SCREEN_TOP + 20,
                       get_minigame_record());
  }
  { // description
    graphics_set_color(WHITE, TRANSP);
    graphics_draw_text(disp, SCREEN_LEFT, RES_Y / 2 + 60,
                       get_minigame_description());
  }
  ...
  { // player state
    ...
    int offset = 38;
    int p1_sprite_offset = !connected_controllers[0]
                               ? SPRITE_player_1_disconnected
                           : players_ready[0] ? SPRITE_player_1_ready
                                              : SPRITE_player_1_not_ready;
    graphics_draw_sprite_trans_stride(disp, start_x, start_y, md_sprites,
                                      p1_sprite_offset);

    int p2_sprite_offset = !connected_controllers[1]
                               ? SPRITE_player_2_disconnected
                           : players_ready[1] ? SPRITE_player_2_ready
                                              : SPRITE_player_2_not_ready;
    graphics_draw_sprite_trans_stride(disp, start_x + offset, start_y,
                                      md_sprites, p2_sprite_offset);

    int p3_sprite_offset = !connected_controllers[2]
                               ? SPRITE_player_3_disconnected
                           : players_ready[2] ? SPRITE_player_3_ready
                                              : SPRITE_player_3_not_ready;
    graphics_draw_sprite_trans_stride(disp, start_x, start_y + offset,
                                      md_sprites, p3_sprite_offset);

    int p4_sprite_offset = !connected_controllers[3]
                               ? SPRITE_player_4_disconnected
                           : players_ready[3] ? SPRITE_player_4_ready
                                              : SPRITE_player_4_not_ready;
    graphics_draw_sprite_trans_stride(disp, start_x + offset, start_y + offset,
                                      md_sprites, p4_sprite_offset);
  }
  ...
  { // ready text
    ...
    if (any_ready) {
      ...
      graphics_draw_text(disp, RES_X / 2 + 60, SCREEN_BOTTOM - 10,
                         "PRESS START");
    }
  }
...
void draw_minigame_controls(display_context_t disp, int start_x, int start_y) {
  ...
  case MINIGAME_FLYINGBATS: {
    start_x += 20;
    // return "Stick\nD-Pad: Move\n\n\nA: Fly";
    graphics_draw_sprite_trans_stride(disp, start_x, start_y, ui_sprites,
                                      SPRITE_joystick);
    graphics_draw_sprite_trans_stride(disp, start_x, start_y + 17, ui_sprites,
                                      SPRITE_dpad);
