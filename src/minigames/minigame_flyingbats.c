...
bool minigame_flyingbats_tick() {
    ...
    for (size_t i = 0; i < MAX_ENEMIES; ++i)
    {
        ...
        if (fb_data->enemies[i].rect.pos.x < -ENEMY_SIZE) {
...
void minigame_flyingbats_display(display_context_t disp) {
    ...
    for (size_t i = 0; i < MAX_ENEMIES; ++i) {
        if (is_intersecting(fb_data->enemies[i].rect, screen_rect)) {
    ...
    for (size_t player_id = 0; player_id < 4; ++player_id) {
        ...
        if (is_intersecting(fb_data->players[player_id].rect, screen_rect)) {
            size_t animCounter = fb_data->players[player_id].speed.y >= 0 ? fb_data->animCounter : fb_data->animCounter*2;
            rdp_sync(SYNC_PIPE);
            rdp_load_texture_stride(0, 0, MIRROR_DISABLED, fb_data->sprites, animCounter % (SPRITE_fb_bat_4 - SPRITE_fb_bat_1 + 1));
            rdp_draw_sprite(0, fb_data->players[player_id].rect.pos.x - 11, fb_data->players[player_id].rect.pos.y - 13, MIRROR_DISABLED);
        }
    }
...
void player_tick(int player_id) {
    ...
    else if (fb_data->players[player_id].rect.pos.y + fb_data->players[player_id].rect.size.height + BORDER_SIZE > RES_Y) {
        fb_data->players[player_id].rect.pos.y = RES_Y - fb_data->players[player_id].rect.size.height - BORDER_SIZE;
    ...
    else if (fb_data->players[player_id].rect.pos.x + fb_data->players[player_id].rect.size.width + BORDER_SIZE > RES_X)
        fb_data->players[player_id].rect.pos.x = RES_X - fb_data->players[player_id].rect.size.width - BORDER_SIZE;
}
